...
from roomsharing.bookings.models import Booking, BookingGroup
    ...
    def handle(self, *args, **options):
        ...
        if sorted_events:
            ...
            for group_index, group in enumerate(sorted_events):
                if group_index >= 300:  # Limit to the first group only
                    break

                g = BookingGroup(title=sorted_events[group][0]['title'])
                print(f"{sorted_events[group][0]['title']} ({len(sorted_events[group])} Bookings)")       
        
      

                for booking_index, booking in enumerate(sorted_events[group]):
                    from datetime import datetime
                    start_dt = datetime.strptime(booking['start_dt'], '%Y-%m-%dT%H:%M:%S%z').strftime('%Y-%m-%d %H:%M')
                    end_dt = datetime.strptime(booking['end_dt'], '%Y-%m-%dT%H:%M:%S%z').strftime('%Y-%m-%d %H:%M')
...
def read_latest_teamup_json():
    ...
    all_files = default_storage.listdir('teamup')[1]  # Das listdir Argument hängt von deiner Speicherstruktur ab
    ...
    latest_file = sorted(teamup_files, key=lambda x: x.split('-')[1].split('.')[0], reverse=True)[0]
